...
var plugin = function (args) {
  return __awaiter(void 0, void 0, void 0, function () {
    var lib, fallbackOk, mediaInfoTracks, trackIdx, vid, anyMaster, anyCll, anySt2086, rawStr, cllZeroMatches, matchCll, matchFall, matchMasterLuminance, ffProbe, _i, _a, st, tags, metaDisplay, metaCll, metaMaxCll, sideData, hasMasteringSideData, hasCllSideData;
    return __generator(this, function (_b) {
      ...
      if (!fallbackOk && args.inputFileObj && args.inputFileObj.ffProbeData) {
        ...
        for (_i = 0, _a = ffProbe; _i < _a.length; _i++) {
          ...
          if (st.codec_type === 'video') {
            tags = st.tags || {};
            // ffprobe keys may vary by source/tooling
            metaDisplay = tags["master-display"] || tags["master_display"] || "";
            metaCll = tags["cll"] || "";
            metaMaxCll = tags["max-cll"] || tags["max_cll"] || "";

            metaDisplay = metaDisplay.toLowerCase();
            metaCll = metaCll.toLowerCase();
            metaMaxCll = metaMaxCll.toLowerCase();

            sideData = st.side_data_list || [];
            hasMasteringSideData = sideData.some(function (sd) {
              var sdType = String((sd && (sd.side_data_type || sd.side_data_name)) || '').toLowerCase();
              return sdType.includes('mastering display metadata');
            });
            hasCllSideData = sideData.some(function (sd) {
              var sdType = String((sd && (sd.side_data_type || sd.side_data_name)) || '').toLowerCase();
              return sdType.includes('content light level metadata');
            });

            if (hasMasteringSideData || hasCllSideData) {
              fallbackOk = true;
              break;
            }

            // If it's "cll=0,0" => missing
            if (metaCll.includes("0,0") || /(^|\D)0\s*,\s*0($|\D)/.test(metaMaxCll)) {
              // do nothing, fallbackOk = false
            } else {
              // If we see numeric >0 => fallbackOk
              if (/\d{3,}/.test(metaDisplay) || /\d{3,}/.test(metaCll) || /\d{3,}/.test(metaMaxCll)) {
                fallbackOk = true;
                break;
              }
            }
      ...
      // Keep a boolean flow variable for downstream plugins.
      args.variables = args.variables || {};
      args.variables.fallbackMissing = !fallbackOk;
      args.jobLog("HDR10 fallback metadata ".concat(fallbackOk ? 'present' : 'missing'));
