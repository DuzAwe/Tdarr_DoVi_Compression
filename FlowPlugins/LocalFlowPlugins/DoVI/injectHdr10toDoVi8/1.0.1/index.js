...
var details = function () { return ({
    ...
    icon: '',
        inputs: [
                { name: 'max_display_mastering_luminance', type: 'string', defaultValue: '1000', inputUI: { type: 'text' },
                    tooltip: 'Max display mastering luminance (nits) written into the generated DoVi RPU.' },
                { name: 'min_display_mastering_luminance', type: 'string', defaultValue: '1', inputUI: { type: 'text' },
                    tooltip: 'Min display mastering luminance (nits) written into the generated DoVi RPU.' },
                { name: 'max_content_light_level', type: 'string', defaultValue: '1000', inputUI: { type: 'text' },
                    tooltip: 'MaxCLL (max content light level, nits) written into the generated DoVi RPU.' },
                { name: 'max_frame_average_light_level', type: 'string', defaultValue: '500', inputUI: { type: 'text' },
                    tooltip: 'MaxFALL (max frame average light level, nits) written into the generated DoVi RPU.' },
        ],
    outputs: [
        {
            number: 1,
        ...
        switch (_a.label) {
            case 0:
                ...
                args.deps.fsextra.writeFileSync(extraJsonPath, JSON.stringify({
                    ...
                    length: 100000,
                    level6: {
                            max_display_mastering_luminance: parseInt(args.inputs.max_display_mastering_luminance, 10) || 1000,
                            min_display_mastering_luminance: parseInt(args.inputs.min_display_mastering_luminance, 10) || 1,
                            max_content_light_level: parseInt(args.inputs.max_content_light_level, 10) || 1000,
                            max_frame_average_light_level: parseInt(args.inputs.max_frame_average_light_level, 10) || 500
                    }
                }, null, 2));
            ...
            case 1:
                ...
                if (!args.deps.fsextra.existsSync(blHevcPath) || args.deps.fsextra.statSync(blHevcPath).size === 0) {
                    args.jobLog('BL.hevc output is missing or empty');
                    throw new Error('ffmpeg produced no BL.hevc output');
                }
            ...
            case 2:
                ...
                if (!args.deps.fsextra.existsSync(rpuBinPath) || args.deps.fsextra.statSync(rpuBinPath).size === 0) {
                    args.jobLog('RPU.bin output is missing or empty');
                    throw new Error('dovi_tool produced no RPU.bin output');
                }
            ...
            case 3:
                ...
                if (!args.deps.fsextra.existsSync(outHevcPath) || args.deps.fsextra.statSync(outHevcPath).size === 0) {
                    args.jobLog('Output HEVC is missing or empty');
                    throw new Error('dovi_tool inject-rpu produced no output');
                }
