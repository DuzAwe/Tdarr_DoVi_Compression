...
const plugin = (args) => {
  ...
  if (!originalPath || !currentPath) {
    const msg = 'Size check failed: unable to resolve file paths';
    if (useX265) {
      args.jobLog(`${msg}; already on x265 fallback, keeping original file`);
  ...
  // If we cannot read either size, fail closed so we don't replace with unknown output.
  if (originalSize <= 0 || currentSize <= 0) {
    const msg = `Size check failed: originalSize=${originalSize} currentSize=${currentSize}`;
    if (useX265) {
      args.jobLog(`${msg}; already on x265 fallback, keeping original file`);
      args.logOutcome('tSuc');
      return {
